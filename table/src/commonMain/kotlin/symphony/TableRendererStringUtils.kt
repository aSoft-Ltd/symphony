...
private fun <D> symphony.table.Table<D>.text(row: Row<D>, col: Column<D>) = when (col) {
    is SelectColumn -> if (selector.isRowSelectedOnCurrentPage(row.number)) "[x]" else "[ ]"
    is DataColumn -> col.resolve(row)
    is ActionColumn -> actions.of(row.item).joinToString(separator = "|") { it.name }
}
private fun <D> symphony.table.Table<D>.text(col: Column<D>) = when (col) {
...
private fun <D> symphony.table.Table<D>.calculateColSizes(gap: Int): MutableMap<Column<D>, Int> {
...
@Deprecated("In favour of other table renderers")
...
fun <D> symphony.table.Table<D>.renderToString(gap: Int = 4) = buildString {
    val colSizes = calculateColSizes(gap)
    val visibleColumns = columns.all().filter { it.visibility.isVisible }
    visibleColumns.forEach { appendRow(text(it), colSizes[it]) }
    appendLine()
