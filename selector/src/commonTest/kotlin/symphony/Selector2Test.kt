...
class Selector2Test {
    ...
    fun should_select_a_row_by_number() = runTest {
        ...
        paginator.initialize { update(persons.all(it)) }
    ...
    fun should_be_able_to_clear_selection_of_the_current_page_only() = runTest {
        ...
        expect(selector.isRowSelectedOnCurrentPage(row = 2)).toBe(
            true,
            "Row 2 / Page 1: was not selected"
        )
        expect(selector.isRowSelectedOnCurrentPage(row = 1)).toBe(
            true,
            "Row 1 / Page 1: was not selected"
        )
        ...
        expect(selector.isRowSelectedOnCurrentPage(row = 2)).toBe(
            true,
            "Row 2 / Page 2: was not selected"
        )
        expect(selector.isRowSelectedOnCurrentPage(row = 1)).toBe(
            true,
            "Row 1 / Page 2: was not selected"
        )

        selector.unSelectAllItemsInTheCurrentPage()

        expect(selector.isRowSelectedOnPage(row = 2, page = 1)).toBe(
            true,
            "Row 2 / Page 1: Was not selected"
        )
        expect(selector.isRowSelectedOnPage(row = 1, page = 1)).toBe(
            true,
            "Row 1 / Page 1: Was not selected"
        )
        ...
        expect(selector.isRowSelectedOnCurrentPage(row = 2)).toBe(
            false,
            "Row 2 / Page 2: was selected"
        )
        expect(selector.isRowSelectedOnCurrentPage(row = 1)).toBe(
            false,
            "Row 1 / Page 2: was selected"
        )
    }
    ...
    fun should_be_able_to_clear_selection_from_all_pages() = runTest {
        ...
        expect(selector.isRowSelectedOnPage(row = 2, page = 1)).toBe(
            false,
            "Row 2 / Page 1: Was still selected after deselection"
        )
        expect(selector.isRowSelectedOnPage(row = 1, page = 1)).toBe(
            false,
            "Row 1 / Page 1: Was still selected after deselection"
        )
    ...
    fun should_be_able_to_select_all_items_in_the_current_page() = runTest {
        ...
        expect(selector.isRowSelectedOnCurrentPage(row = 2)).toBe(
            false,
            "Row 2 / Page 2: was already selected"
        )
        expect(selector.isRowSelectedOnCurrentPage(row = 1)).toBe(
            false,
            "Row 1 / Page 2: was already selected"
        )
    ...
    fun should_be_able_to_toggle_selection_of_current_page() = runTest {
        ...
        expect(selector.isRowSelectedOnCurrentPage(row = 1)).toBe(
            true,
            "Row 1 / Page 2: was supposed to be selected"
        )

        selector.toggleSelectionOfRowInCurrentPage(row = 1)
        expect(selector.isRowSelectedOnCurrentPage(row = 1)).toBe(
            false,
            "Row 1 / Page 2: was supposed to not be selected"
        )
    }
    ...
    fun should_have_zero_actions_after_all_rows_in_page_have_been_unselected() = runTest {
        ...
        repeat(4) {
            expect(selector.isRowSelectedOnPage(row = it + 1, page = 1)).toBe(
                true,
                "Row ${it + 1} / Page 1: was supposed to be selected"
            )
        }
