...
class SelectorBasedActionManager2Test {

    @Test
    fun should_be_able_to_dynamically_add_primary_actions() = runTest {
        val persons = Persons(total = 15)
        val pag = paginator<Person>(10)
        pag.initialize { params -> update(persons.all(params)) }
    ...
    fun should_not_crash_if_there_is_a_selected_item_and_there_are_no_current_actions() = runTest {
        val person = Persons(total = 15)
        val pag = paginator<Person>(10)
        pag.initialize { params -> update(person.all(params)) }
        val sel = selectorOf(pag)
        val actions = actionsOf(sel) {
            primary {
                onCreate { println("Create things") }
